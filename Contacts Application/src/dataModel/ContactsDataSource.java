...
//            conn=DriverManager.getConnection(CONNECTION_STRING,"root","");
    ...
    public boolean createTableContact(){
     ...
     public boolean insertData(String fname,String lname,String mno,String email){
        try {
            Statement statement=conn.createStatement();
            statement.execute("INSERT INTO "+TABLE_CONTACT+" VALUES ('"+fname+"', '"+lname+"', '"+mno+"', '"+email+"')");
            return true;
        }catch (SQLException e){
            System.out.println(e.getMessage());
            e.printStackTrace();
     ...
     public List<ContactDataModel> queryContact(){
        try {
            Statement statement=conn.createStatement();
            ResultSet result=statement.executeQuery("SELECT * FROM "+TABLE_CONTACT);
